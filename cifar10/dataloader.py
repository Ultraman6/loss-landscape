...
from protonet.prototypical_batch_sampler import init_sampler
...
def load_dataset(args, batch_size=128, \
    ...
    if data_split > 1:
        ...
        if args.proto:
            train_sampler = init_sampler(args, trainset.targets, indices, 'train')
            train_loader = torch.utils.data.DataLoader(trainset, batch_sampler=train_sampler)
        else:
        ...
        if args.proto:
            train_sampler = init_sampler(args, trainset.targets, None, 'train')
            train_loader = torch.utils.data.DataLoader(trainset, batch_sampler=train_sampler)
        else:
            train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
    ...
    #     test_loader = torch.utils.data.DataLoader(testset, batch_sampler=test_sampler)
    # else:
